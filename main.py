...
class Biblioteka:
  ...
  def obliczanie_pozycji(self, ksiazki_lista, egzemplarze_lista):
...
n=int(input())
for i in range (0,n):
  eg=input().replace('(', '').replace(')', '').split(",")
  pozycje.append(eg)
  biblioteka.dodaj_egzemplarz(pozycje[i][0],pozycje[i][1],pozycje[i][2])
  


biblioteka.obliczanie_pozycji(sorted(biblioteka.ksiazki_lista,key=lambda ksiazka:ksiazka.tytul),biblioteka.egzemplarze_lista)

for i in range(len(ksiazki_lista)):
    liczba=0
    k=ksiazki_lista[i]
    for i in range(len(self.egzemplarze_lista)):
      eg = egzemplarze_lista[i]
